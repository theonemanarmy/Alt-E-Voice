    ...
    <div class="card card-register mx-auto mt-5">
      <div class="card-header">
        ...
        <?php if (isset($user) && isset($edit) && is_object($user)) : ?>
          ...
          Editar mis datos
          <?php $url_action =  url_project."usuario/save&id=".$user->id; ?>
        <?php else : ?>
          Registrar una cuenta
          <?php $url_action = url_project."usuario/save"; ?>
        <?php endif; ?>
        ...
        <form action="<?=$url_action?>" method="POST" enctype="multipart/form-data">
          ...
          <div class="form-group">
            <div class="form-label-group" data-aos="zoom-in-right">
              <input type="email" name="email"  value="<?= isset($user) && is_object($user) ? $user->email : ''; ?>" id="inputEmail" onkeyup="this.className = 'campo';" class="form-control campo" placeholder="E-mail" required="required" minlength="5" maxlength="150" pattern="^[a-z0-9!#$%&'*+/=?^_`{|}~-]+(?:\.[a-z0-9!#$%&'*+/=?^_`{|}~-]+)*@(?:[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\.)+[a-z0-9](?:[a-z0-9-]*[a-z0-9])?$"><span></span>
